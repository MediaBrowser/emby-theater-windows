...
$process = (Start-Process -FilePath $7za -ArgumentList "$7zaOptions ""$DeployPath\emby.theater.zip"" ""$DeployPathServerPath\*""" -Wait -Passthru -NoNewWindow -RedirectStandardOutput "$PSScriptRoot\7za.log" -RedirectStandardError "$PSScriptRoot\7zaError.log").ExitCode
...
$WindowsReleaseVersion = ((Get-Command "$WindowsBinReleasePath\Emby.Theater.exe").FileVersionInfo).FileVersion
Write-Host "Windows Release: Copy archive to emby.theater-$WindowsReleaseVersion.zip..."
Copy-Item -Path "$DeployPath\emby.theater.zip" -Destination "$DeployPath\emby.theater-$WindowsReleaseVersion.zip" -Force
